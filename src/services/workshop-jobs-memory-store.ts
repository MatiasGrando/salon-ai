...
import { isWorkshopDefaultShortcut, mergeWorkshopShortcuts } from './workshop-shortcuts.js'
export function createWorkshopJobsMemoryStore(
  ...
  return {
    ...
    async create(businessId,job){const vehicle=findVehicle(businessId,job.vehicleId);if(!vehicle)throw new WorkshopJobError('Auto no encontrado en este taller');const validServices=new Set(mergeWorkshopShortcuts(businessId,shortcuts.filter(x=>x.businessId===businessId)).map(x=>x.id));if(job.lines.some((line:any)=>line.serviceId&&!validServices.has(line.serviceId)))throw new WorkshopJobError('Uno de los servicios no pertenece a este taller');const performer=performers.find(x=>x.businessId===businessId&&x.id===job.performerId&&x.active);if(!performer)throw new WorkshopJobError('Seleccioná un trabajador activo');const record={id:randomUUID(),businessId,...job,responsible:performer.name,createdAt:new Date().toISOString(),vehicle:{id:vehicle.id,plate:vehicle.plate,model:vehicle.model}};jobs.push(record);vehicle.currentMileage=Math.max(vehicle.currentMileage??0,job.mileage);return record}
  }
}
