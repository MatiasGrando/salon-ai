...
const fakeService = new Proxy({}, {
  get: (_target, property) => async (input: Record<string, unknown>) => {
    ...
    if (property === 'getCurrentCashRegister') return { day: { id: 'active-day', closedAt: null }, session: { id: 'active-session' }, summary: null }
    if (property === 'listCashRegisterDays') return [{ id: 'old-day', closedAt: new Date() }, { id: 'active-day', closedAt: null }]
...
const staffDays = await app.inject({ method: 'GET', url: '/cash-register/days', headers: { 'x-view': 'yes' } })
assert.deepEqual(staffDays.json().days.map((day: { id: string }) => day.id), ['active-day'])
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/days/old-day/summary', headers: { 'x-view': 'yes' } })).statusCode, 404)
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/days/old-day/entries', headers: { 'x-view': 'yes' } })).statusCode, 404)
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/period/summary?from=2026-09-01&to=2026-09-30', headers: { 'x-view': 'yes' } })).statusCode, 403)
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/period/expenses?from=2026-09-01&to=2026-09-30', headers: { 'x-view': 'yes' } })).statusCode, 403)
const adminDays = await app.inject({ method: 'GET', url: '/cash-register/days', headers: { 'x-role': 'BUSINESS_ADMIN' } })
assert.deepEqual(adminDays.json().days.map((day: { id: string }) => day.id), ['old-day', 'active-day'])
...
assert.equal((await app.inject({ method: 'POST', url: '/cash-register/open', headers: { 'x-role': 'SUPER_ADMIN' }, payload: { businessId: 'business-a', responsibleUserId: 'user' } })).statusCode, 200)
assert.deepEqual(callInputs.findLast((call) => call.property === 'openRegisterDay')?.input.actingAdministrator, { id: 'user', name: 'User' })
assert.equal((await app.inject({ method: 'POST', url: '/cash-register/open', headers: { 'x-sessions': 'yes' }, payload: { responsibleUserId: 'user' } })).statusCode, 200)
assert.equal(callInputs.findLast((call) => call.property === 'openRegisterDay')?.input.actingAdministrator, undefined)
...
assert.equal((await app.inject({ method: 'POST', url: '/cash-register/new-session', headers: { 'x-role': 'BUSINESS_ADMIN' }, payload: { currentSessionId: 'session', responsibleUserId: 'user', countedCash: 90 } })).statusCode, 200)
assert.deepEqual(callInputs.findLast((call) => call.property === 'startNewSession')?.input.actingAdministrator, { id: 'user', name: 'User' })
...
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/period/summary', headers: { 'x-role': 'BUSINESS_ADMIN' } })).statusCode, 400)
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/period/summary?from=2026-09-01&to=2026-09-30', headers: { 'x-role': 'BUSINESS_ADMIN' } })).statusCode, 200)
...
assert.equal((await app.inject({ method: 'GET', url: '/cash-register/period/expenses?from=2026-09-01&to=2026-09-30&page=2&pageSize=10&method=CASH&categoryId=category-1&q=luz', headers: { 'x-role': 'BUSINESS_ADMIN' } })).statusCode, 200)
