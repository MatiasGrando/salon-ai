...
assert.doesNotMatch(cashUi, /createDepositMethod\.value = 'CASH'/, 'la seña nueva no debe volver a Efectivo')
assert.match(cashUi, /createDepositMethod\.value = 'TRANSFER'/, 'Transferencia debe ser el medio predeterminado para la seña al crear el turno')
assert.doesNotMatch(cashUi, /editDepositMethod\.value = 'CASH'/, 'la seña sobre un turno existente no debe volver a Efectivo')
assert.match(cashUi, /editDepositMethod\.value = 'TRANSFER'/, 'Transferencia debe ser el medio predeterminado para la seña de un turno existente')
assert.match(cashUi, /id="appointment-create-deposit-method"[\s\S]{0,180}<option value="TRANSFER" selected>/, 'el formulario nuevo debe renderizar Transferencia seleccionada')
assert.match(cashUi, /id="appointment-edit-deposit-method"[\s\S]{0,180}<option value="TRANSFER" selected>/, 'el formulario de edición debe renderizar Transferencia seleccionada')
