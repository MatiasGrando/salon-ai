...
try {
  ...
  const optimizedMobileHeroImage = await readFile('sites/peluqueria-glow-web/public/hero-glow-mobile-v2.webp')
  assert.equal(mobileHeroImage.subarray(0, 8).toString('hex'), '89504e470d0a1a0a')
  assert.equal(optimizedMobileHeroImage.subarray(0, 4).toString('ascii'), 'RIFF')
  ...
  await writeFile(join(dir, 'hero-glow-mobile-v2.webp'), optimizedMobileHeroImage)
  ...
  const optimizedMobileImage = await app.inject({ url: '/hero-glow-mobile-v2.webp', headers: { host: 'glow.weex.com.ar' } })
  assert.equal(optimizedMobileImage.statusCode, 200)
  assert.equal(optimizedMobileImage.headers['content-type'], 'image/webp')
  assert.match(optimizedMobileImage.headers['cache-control']!, /max-age=31536000/)
  assert.match(optimizedMobileImage.headers['cache-control']!, /immutable/)
  assert.deepEqual(optimizedMobileImage.rawPayload, optimizedMobileHeroImage)
  assert.equal((await app.inject({ method: 'HEAD', url: '/hero-glow-mobile-v2.webp', headers: { host: 'glow.weex.com.ar' } })).statusCode, 200)
  assert.equal(isGlowSitePublicResource('GET', 'glow.weex.com.ar', '/hero-glow-mobile-v2.webp'), true)
