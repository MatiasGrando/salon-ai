...
assert.ok(route.includes('appendDemoPreview(`${basePath}/reservar`, demoPreview)'), 'la landing real debe conservar el acceso al flujo de reservas')
assert.ok(route.includes("if (!demoPreview) return url\n  return '#plantilla-modelo'"), 'la plantilla modelo no debe ingresar al flujo de reservas real')
