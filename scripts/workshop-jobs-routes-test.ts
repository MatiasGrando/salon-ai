...
assert.equal(defaults.json().length,8)
assert.equal(defaults.json().find((x:any)=>x.name==='Cambio de aceite').recurrenceEnabled,true)
assert.equal(defaults.json().find((x:any)=>x.name==='Cambio de aceite').returnMonths,6)
assert.equal(defaults.json().find((x:any)=>x.name==='Cambio de aceite').returnKilometers,10000)
const defaultId=defaults.json()[0].id
let edited=await app.inject({method:'PATCH',url:'/workshop/shortcuts/'+defaultId,payload:{businessId:'a',name:'Aceite premium',description:'Cambio premium',quantity:1,active:false,recurrenceEnabled:true,returnMonths:8,returnKilometers:12000,customerInstructions:'Revisar nivel'}})
...
assert.equal(defaults.json().find((x:any)=>x.id===defaultId).returnMonths,8)
assert.equal(defaults.json().find((x:any)=>x.id===defaultId).returnKilometers,12000)
...
const linkedServiceId=defaults.json().find((x:any)=>x.active).id
const body={...baseBody,performerId:anaId,lines:[{...baseBody.lines[0],serviceId:linkedServiceId}]}
r=await app.inject({method:'POST',url:'/workshop/jobs',payload:body});assert.equal(r.statusCode,201,r.body);assert.equal(r.json().totalCents,0);assert.equal(r.json().responsible,'Ana');assert.equal(r.json().performerId,anaId);assert.equal(r.json().lines[0].serviceId,linkedServiceId);assert.equal(vehicle.currentMileage,85000)
...
r=await app.inject({method:'POST',url:'/workshop/jobs',payload:{...body,lines:[{...body.lines[0],serviceId:'foreign-service'}]}});assert.equal(r.statusCode,400)
...
r=await app.inject({method:'GET',url:'/workshop/jobs?businessId=a&limit=10&offset=0'});assert.equal(r.statusCode,200,r.body);assert.equal(r.json().items.length,10);assert.equal(r.json().hasMore,true);assert.equal(r.json().nextOffset,10)
r=await app.inject({method:'GET',url:'/workshop/jobs?businessId=a&limit=10&offset=10'});assert.equal(r.statusCode,200,r.body);assert.equal(r.json().items.length,5);assert.equal(r.json().hasMore,false);assert.equal(r.json().nextOffset,null)
...
defaults=await app.inject({method:'GET',url:'/workshop/shortcuts?businessId=b'});assert.equal(defaults.json().length,8);assert.ok(defaults.json().every((x:any)=>x.active));assert.ok(!defaults.json().some((x:any)=>x.id===defaultId))
