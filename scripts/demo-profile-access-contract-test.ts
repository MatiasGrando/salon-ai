...
assert.match(demoRoute, /SHARED_SALES_DEMO_TYPES = \['NAILS', 'HAIR_SALON'\]/)
assert.match(demoRoute, /\['SUPER_ADMIN', 'ACCOUNT_ADMIN'\]\.includes\(role\)/)
...
assert.match(authGuard, /demoType: \{ in: \['NAILS', 'HAIR_SALON'\] \}/)
...
assert.match(crmUi, /\['SUPER_ADMIN', 'ACCOUNT_ADMIN'\]\.includes\(state\.currentUser\?\.role\)/)
