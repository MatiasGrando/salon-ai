...
assert.match(authGuard, /demo-profiles\\\/\[\^\/\]\+\\\/access/)
assert.match(authGuard, /demo-profiles\\\/\[\^\/\]\+\\\/chat/)
assert.match(authGuard, /SHARED_COMMERCIAL_DEMO_TYPES = \['NAILS', 'HAIR_SALON', 'BARBERSHOP', 'PILATES'\]/)
assert.match(authGuard, /authorizeCommercialDemoWorkspace/)
assert.match(authGuard, /isAccountAdminBusinessWorkspaceRoute\(method, path\)/)
assert.match(authGuard, /auth\.user\.role === 'ACCOUNT_ADMIN' \|\| auth\.user\.canCreateBusinesses/)
assert.match(authGuard, /isDemo: true/)
assert.match(authGuard, /demoType: \{ in: \[\.\.\.SHARED_COMMERCIAL_DEMO_TYPES\] \}/)
assert.match(authGuard, /authorizedBusinessIdOverride/)
assert.doesNotMatch(authGuard, /SHARED_COMMERCIAL_DEMO_TYPES[^\n]*QA_SANDBOX/)
assert.match(authGuard, /demoType: 'QA_SANDBOX'/)
assert.match(authGuard, /if \(qaSandbox\) return false/)
assert.match(authGuard, /path === '\/professionals'/)
assert.match(authGuard, /path === '\/services'/)
assert.match(authGuard, /path === '\/crm\/ai-settings'/)

assert.match(businessRoute, /canCreateBusiness\(request\.auth\.user\)/, 'La creación de cuentas debe conservar su permiso específico.')
assert.match(businessRoute, /businessAccessWhere\(resolveBusinessScope\(request\.auth\.user\)\)/, 'El listado de cuentas reales debe respetar la asignación actual.')
