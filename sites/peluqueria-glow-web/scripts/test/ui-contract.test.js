...
test('both sections share branch state and are remounted to clear profiles and expanded cards',()=>{
 ...
 for (const [component, keyPrefix] of [['ServicesSection','services'],['ProfessionalsSection','professionals']]) {
  const props=app.match(new RegExp(`<${component}([\\s\\S]*?)/>`))[1];
  assert.match(props,new RegExp('key=\\{`' + keyPrefix + '-\\$\\{selectedBranch\\}`\\}'));
